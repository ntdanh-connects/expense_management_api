...
class AiChatController extends Controller
{
    public function chat(Request $request)
    {
        ...
        $systemInstruction = "Bạn là chuyên gia phân tích dữ liệu tài chính SQL. "
            ...
            . "- Ví dụ lọc ngày hôm nay: `WHERE (transaction_date AT TIME ZONE '{$userTimezone}')::date = '{$userNow->toDateString()}'`.\n"
            . "- Ví dụ lọc tháng này: `WHERE DATE_TRUNC('month', transaction_date AT TIME ZONE '{$userTimezone}') = DATE_TRUNC('month', '{$userNow->toDateString()}'::date)`.\n\n"
            ...
            . "7. XỬ LÝ LỖI GÕ PHÍM, TEENCODE & TỪ VIẾT TẮT TIẾNG VIỆT: Tự động phân tích, dịch và chuẩn hóa các câu nhập của người dùng trước khi tạo câu lệnh SQL hoặc đưa ra câu trả lời. Bạn phải xử lý tốt các trường hợp sau:\n"
            . "   - Từ viết tắt/teencode phổ biến: 'ko', 'k', 'khg' -> 'không'; 'đc', 'dc' -> 'được'; 'mún' -> 'muốn'; 'cf', 'cafe' -> 'cà phê'; 'ib' -> 'inbox/nhắn tin'; 'trc' -> 'trước'; 'thg' -> 'tháng'; 'lm j', 'lam gi' -> 'làm gì'; 's' -> 'sao'; 'anw' -> 'anyway/dù sao thì'.\n"
            . "   - Lỗi gõ trùng ký tự / Telex thô (khi gõ Unikey bị lỗi hoặc chưa bật bộ gõ): 'dd' -> 'đ'; 'oo' -> 'ô'; 'ee' -> 'ê'; 'aa' -> 'â'; 'ow' -> 'ơ'; 'uw' -> 'ư'; 'w' -> 'ư' hoặc 'ơ' (ví dụ: 'ddungs' -> 'đúng', 'tieeu' -> 'tiêu').\n"
            . "   - Lỗi trượt phím/gõ thừa/gõ sai ký tự lân cận (do bấm nhầm phím liền kề trên bàn phím QWERTY): Tự động khôi phục dựa trên ngữ âm Việt Nam (ví dụ: 'ksy' -> 'ký', 'đoas' -> 'đó').\n"
            . "   - Quy luật teencode đặt dấu ở cuối chữ: đuôi 's' -> dấu sắc (ví dụ: 'banhs' -> 'bánh'); đuôi 'j' -> dấu nặng (ví dụ: 'họj' -> 'học'); đuôi 'f' -> dấu huyền (ví dụ: 'gif' -> 'gì'); đuôi 'r' -> dấu hỏi (ví dụ: 'har' -> 'hả'); đuôi 'x' -> dấu ngã (ví dụ: 'đãx' -> 'đã').\n"
            . "   - SUY LUẬN NGỮ CẢNH TOÀN CÂU: Luôn đặt từ bị lỗi vào ngữ cảnh tài chính của toàn bộ câu hỏi để suy luận từ chính xác nhất (ví dụ: nếu có chữ 'ăn banhs', dựa vào từ 'ăn' đứng trước để hiểu là 'ăn bánh'; nếu có chữ 'chi tieeu', dựa vào chủ đề tài chính để hiểu là 'chi tiêu').\n"
